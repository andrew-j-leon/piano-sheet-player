...
FMODError::FMODError(const FMOD_RESULT& theResult)
	: result(theResult)
{
}


void FMODError::message() const
{
	std::cout << resultToCharStr(result) << std::endl;
}


const char* FMODError::resultToCharStr(const FMOD_RESULT& errcode) const
{
	switch (errcode)
	{
		/*$ preserve end $*/
